-- Privileged reactivation of a Superseded checklist version. Authenticated
-- roles keep SELECT-only table access; writes go through this SECURITY DEFINER
-- RPC that authorizes Admin/Manager, requires status='superseded', matches
-- expected_revision and a required non-null expected_active_version_id, does
-- not mutate Items, and commits Active + Supersede + one reactivated event.
...
CREATE OR REPLACE FUNCTION public.reactivate_checklist_version(
  version_id uuid,
...
BEGIN
  ...
  THEN
    RAISE EXCEPTION 'Not authorized to reactivate a checklist version'
      USING ERRCODE = '42501';
  END IF;
  ...
  -- Same (bike_category, phase) lock used by draft create, item writes, and activate.
  ...
  IF v_status IS DISTINCT FROM 'superseded' THEN
    RAISE EXCEPTION 'Checklist version is not superseded'
      USING ERRCODE = '55000',
  ...
  IF v_active_id IS NULL
     OR v_revision IS DISTINCT FROM v_expected_revision
...
COMMENT ON FUNCTION public.reactivate_checklist_version(uuid, integer, uuid) IS
  'Admin/Manager-only transactional reactivation. Locks (bike_category, phase), requires superseded status and a matching non-null Active pointer, does not mutate Items, and commits Active + Supersede + one reactivated event.';

REVOKE ALL ON FUNCTION public.reactivate_checklist_version(uuid, integer, uuid)
  FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.reactivate_checklist_version(uuid, integer, uuid)
  TO authenticated;
