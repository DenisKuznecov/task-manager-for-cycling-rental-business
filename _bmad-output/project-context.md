...
- **Canonical fetch include:** `src/lib/booqable/canonical-adapter.ts` owns the nested-order include `customer,coupon,lines.planning.stock_item_plannings.stock_item.barcode`. Live `sync.ts` still uses `customer,coupon,lines` until cutover. Standalone StockItem collections stay unverified and must not become the only path. The database coordinator `apply_canonical_order_graph` is the sole canonical-source writer; do not change `sync.ts`, the webhook, or sandbox routes until a later cutover story.
...
- **Booqable refresh model (v1)** — an authenticated webhook and any consequential JIT caller must re-fetch Booqable authority and apply it atomically. The payload is signal-only: never use its fields as current truth. Successfully processed duplicate, delayed, or out-of-order updates must be idempotent. Failed or missed signals have no application-managed retry queue, worker, reconciliation sweep, manual recovery API, or direct source/task-table repair. The existing secret-protected, preview-denied `GET /api/sandbox/booqable/sync-orders` is a temporary legacy exception: it refetches Booqable authority through the existing sync path and does not directly edit source or task tables. It creates no new per-order/manual recovery API, retry queue, worker, Cron job, or reconciliation system; retire or further contain it only through a future explicitly approved replacement-or-removal decision.
- **The Booqable webhook is intentionally "thin"** (`src/app/api/webhooks/booqable/route.ts`): use the payload only to identify which order changed and filter out "ghost" orders (`status: new/concept`), then re-fetch authoritative order/customer/item data via `syncBooqableOrder`. Auth remains a static `?secret=` query parameter against `BOOQABLE_WEBHOOK_SECRET`, not HMAC signature verification. Keep webhook handling within the Vercel Hobby 10-second execution limit.
