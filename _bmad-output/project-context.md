...
- **Booqable** — external rental management platform; source of truth for orders/inventory, integrated via webhook (`src/app/api/webhooks/booqable`) and sync logic (`src/lib/booqable/sync.ts`). Versioned source envelopes and the six-value apply-result vocabulary live in `src/lib/booqable/contracts/` (Zod source, fixture-checked PostgreSQL enums). Bike classification and Setup mapping live in the same package (`classification-config.ts`): empty ProductGroup UUID allowlist fails closed, labels are display-only, and broad `review_updated_configuration` is the only selectable mode until all five Setup Categories are fixture-proven. Runtime reads the Active database snapshot, not live file edits. Existing writers are unchanged — `sync.ts`, the webhook, and sandbox routes do not consume this package yet.
...
- Unit tests use **Vitest** (`npm run test:unit`). Existing coverage is in `tests/booqable-containment/`, `tests/booqable-contracts/`, `tests/runtime-upgrade/`, and `tests/toolchain/`. `npm run contracts:check` is the named envelope drift/compatibility surface (also run by `.github/workflows/contracts-drift.yml` on pull_request). Database proof is pgTAP under nested `supabase/tests/database/` (`workshop-tasks/`, `toolchain/`, `booqable-integration/`); the pinned CLI must still discover those trees via `supabase test db`. Don't add Jest/Playwright or a second runner unprompted.
...
- **Classification mapping** is owned by `src/lib/booqable/contracts/classification-config.ts` plus admin-only approve/rollback RPCs. Allowlist keys are actual ProductGroup UUIDs with provenance; an empty allowlist is valid and fail-closed. Labels and analyst-candidate names never classify. Broad `review_updated_configuration` is the v1 default; do not activate `targeted` until every Setup Category (`pedals`, `saddle`, `wheelset`, `power-meter`, `computer-mount`) has a stable approved identifier and complete null/unknown/changed/removed fixtures. Do not invent UUIDs or Booqable field names to fill those slots.
