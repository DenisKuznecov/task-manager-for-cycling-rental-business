...
- Classify bikes only from immutable, business-approved ProductGroup UUID allowlists. Labels and analyst names never classify. Unmapped trackable groups fail closed. Title matching is forbidden.
- Setup Categories are Pedals, Saddle, Wheelset, Power meter, and Computer mount. Targeted invalidation stays off until every category has approved stable identifiers and fixtures for null, unknown, changed, and removed. Until then, broad configuration-review mode is the safe default.
...
- Evolve existing `customers`, `orders`, and `order_items` additively. Admit only approved ProductGroups/Products, fixture-proven Bundles when required, Plannings, StockItemPlannings, StockItems, immutable membership roots, source-version state, inbox, reconciliation checkpoints, and incidents. No second Workshop projection and no raw-payload mirror. Referenced rows archive or close; they do not cascade-delete.
...
- The coordinator is the sole canonical-source writer. This epic applies source atomically with derivation disabled: observed source advances, debt is retained, result is `derivation_disabled`. No direct source or task table patches.
...
- Prove locally with adapter fixtures, pgTAP for envelope/comparator/privileges, and a multi-session overlapping-ingestion harness.

## Cross-Story Dependencies

- Epic 1’s active templates must exist before later derivation; membership and Bike Task creation belong to Epic 3.
- Sequence: containment and toolchain → envelopes and mapping config → additive projection and brownfield preservation → durable recovery and workers → atomic apply and JIT proofs → caller cutover → revoke writers → rollout control plane.
