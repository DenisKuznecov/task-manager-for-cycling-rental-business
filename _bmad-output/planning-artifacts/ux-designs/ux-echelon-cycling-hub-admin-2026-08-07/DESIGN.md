...
- `task-context-panel` — Narrower landscape panel for bike identity, order, customer, delivery address, rental dates, current setup values, accessories, Notes, and current `extra_information`. Current `No` values have the same weight as selected values.
...
- `template-editor` — Manager/admin workspace for template metadata and continuously visible Item definitions. Inputs use Workshop-sized heights; version and status remain visible while editing.
