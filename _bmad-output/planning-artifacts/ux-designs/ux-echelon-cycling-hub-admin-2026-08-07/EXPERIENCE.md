...
| `task-context-panel` | Bike Task Detail | Shows physical-bike identity first, then order/customer/dates/address, setup, accessories, Notes, and current `extra_information` with source labels. Booqable-owned values are read-only. `No` setup values remain fully visible. |
